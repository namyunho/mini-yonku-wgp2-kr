...
순서:
 ...
 7. build_worldmap.py    월드맵 퀴즈 70문항(350문자열) $C6 in-bank 재배치+포인터 패치
 8. build_sjis.py        SJIS UI(→out/menu_test.smc) → diff를 out/wgp2_kr.smc에 통합
 9. BPS 배포 패치 생성(flips)
...
def main():
    run(["scripts/build_patch.py", "--adv-json", ADV_JSON,
         "--worldmap-json", WORLDMAP_JSON])                            # 1 (공유 $CA 글리프 할당)
    ...
    run(["scripts/build_worldmap.py"])                                 # 7 월드맵 퀴즈 → out
    run(["scripts/build_sjis.py"])                                     # 8 SJIS UI 한글화 → menu_test.smc

    # 8b: SJIS 패치(원본 대비 변경 바이트)를 통합 ROM에 적용.
