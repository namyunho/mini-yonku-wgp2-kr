...
CMD_PROG = {
    ...
    0x47: [('expr',)], 0x50: [('b', 16)], 0x51: [('list0',)], 0x52: [], 0x53: [('b', 1)],
...
# 표현식 서브옵 -> 인자 바이트 수 (표 $C0:5B7C, $C0:5B4A 가 0x00 만날 때까지 루프).
#   sub 01 = push.w(2B), sub 02 = push.b(1B), 나머지 대다수는 스택 연산자(0B).
EXPR_ARG = {
...
def read_expr(buf, p):
    """$C0:5B4A 표현식: 서브옵을 0x00 만날 때까지 소비."""
    while p < len(buf):
        sub = buf[p]; p += 1
        if sub == 0x00:
            return p
        p += EXPR_ARG.get(sub, 0)
    return p


def consume(buf, p, cmd):
    """명령 바이트 다음 위치 p 에서 오퍼랜드를 소비하고 다음 명령 위치를 반환."""
    for step in CMD_PROG[cmd]:
        if step[0] == 'b':
            p += step[1]
        elif step[0] == 'expr':
            p = read_expr(buf, p)
...
def walk(buf, start=0, limit=None):
    """스크립트를 선형 워크. 텍스트런과 desync를 보고."""
    p = start
    runs = []
