...
| **어드벤처 스토리 번역·재삽입** | ✅ **완료** — 번역 + **위치보존 크래시 원천차단** (런 단위 패딩 → 디컴프 스크립트 길이 불변 → VM offset 보존, [docs/14](docs/14-position-preserving-translation.md)). 긴 런 Codex 3라운드 축약, cmd0x20 메뉴/선택지 한글화 포함. **오프닝~엔딩 전편 실기 완주(크래시 0) 확인** |
| **월드맵 퀴즈·정보 DB** | ✅ 70문항·350문자열 추출/번역/재삽입 완료 (`$C6:A08D` 포인터표, 350/350 왕복·역디코드 — [docs/19](docs/19-worldmap-quiz-text.md)) |
| **장소별 필드/NPC 숨은 레코드** | 🟡 전수 발굴·**번역 원장 완료** (**1,207레코드**, C2 참조 1,300개, 텍스트런 1,411/고유 **1,340 전량 번역** — 위치보존 재삽입·실기 QA 대기, [docs/20](docs/20-field-npc-hidden-records.md)·[docs/21](docs/21-field-position-preserving-translation.md)) |
...
> **번역 현황 요약**: 정적 대사·어드벤처 스토리(오프닝~엔딩 전편)·월드맵 퀴즈/정보 DB·**장소별 필드/NPC 대사 1,340종**·비압축 **SJIS 메뉴/UI**(레이서·팀·머신/파츠·행성·옵션·저장메뉴)·**소형 타일폰트 메뉴**(월드맵 X메뉴·조작방법·용어집·지도·수동 세팅)의 번역 원장이 완성됐다. 필드/NPC는 위치보존 재삽입과 실기 QA가 남았으며, 타일에 구워진 그래픽 텍스트도 아래 [남은 작업](#남은-작업)과 같이 남아 있다.
...
| 장소별 필드/NPC 번역 원장 | **텍스트런 1,411 / 고유 1,340** | `text_kr_full` 완역본과 `text_kr` 삽입본 전량 작성, 제어마커·줄수·줄폭≤16·위치 식별자 감사 통과 |
...
> 정적 대사·어드벤처 스토리(전편)·월드맵 퀴즈·필드/NPC 번역 원장·SJIS 메뉴/UI·소형 타일폰트 메뉴는 번역이 끝났다.
> 남은 항목은 아래와 같으며, 진행하며 추가로 발견될 수 있다.

**텍스트 번역**
- **장소별 필드/NPC 대사 재삽입·실기 QA** — C4/C5/C6 씬간 압축 팩의 **1,340 고유레코드 번역 원장 완료**(`field_kr.json`). `build_all.py`에는 아직 필드 삽입 단계가 연결되지 않았으므로, 위치보존 빌더 구현·통합 후 대표 장소 실기 확인이 필요([docs/21](docs/21-field-position-preserving-translation.md)).
