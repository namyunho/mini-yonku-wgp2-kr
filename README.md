...
> **번역 현황 요약**: 정적 대사·어드벤처 스토리(오프닝~엔딩 전편)·월드맵 퀴즈/정보 DB·비압축 **SJIS 메뉴/UI**(레이서·팀·머신/파츠·행성·옵션·저장메뉴)·**소형 타일폰트 메뉴**(월드맵 X메뉴·조작방법·용어집·지도·수동 세팅)까지 한글화가 끝났다. 남은 항목은 **장소별 필드/NPC 대사**(1,340 고유레코드 전수 추출 완료, Codex 번역 진행중)와 **타일에 구워진 그래픽 텍스트**(엔딩 크레딧·챕터 타이틀·Result 순위·일시정지 메뉴·개러지 파츠/포메이션/데미지 이미지 폰트 등)다 — 아래 [남은 작업](#남은-작업) 참조.
...
> 정적 대사·어드벤처 스토리(전편)·월드맵 퀴즈·SJIS 메뉴/UI·소형 타일폰트 메뉴는 한글화가 끝났다.
> 남은 항목은 아래와 같으며, 진행하며 추가로 발견될 수 있다.

**텍스트 번역**
- **장소별 필드/NPC 대사** — Codex 진행중. C4/C5/C6 씬간 압축 팩에 분산된 **1,340 고유레코드**를 전수 추출(`field_text.json`)했고, 위치보존 재삽입([docs/21](docs/21-field-position-preserving-translation.md))으로 번역 진행.
