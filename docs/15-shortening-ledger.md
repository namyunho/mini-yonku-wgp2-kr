...
| 시스템 | 키 | 축약 이유 |
|--------|-----|-----------|
| **어드벤처**(`adventure_kr.json`) | `scene`(hex)·`at` | **위치보존**(docs/14): cmd0x21 런을 원본 바이트 이하로 축약해야 말미 공백 패딩으로 런 길이를 원본과 일치시켜 VM offset(조건분기·컨테이너)을 보존한다. 한글이 원본보다 길면 패딩 불가 → 축약 필수. |
...
어드벤처 위치보존 확대 시 축약할 **긴 런**(한글 인코딩 > 원본 바이트). 목록·before 텍스트는
`shortening_ledger.json`의 `pending` 및 빌드 산출 `out/retranslate_longer.json`에 있다.
over 분포: +1B 127 · +2B 110 · +3B 68 · +4B 48 · +5B 34 · +6B 27 · +7B 15 · +8~12B 17.
