...
## 🔴 다음 최우선 (남은 피드백)
4. **포메이션 설정 안내 대사** — ✅ **부분 완료(2026-07-21)**. **규명·수정·실기통과**: 파서 `$C1:9554`가 표시하는 **고아 문자열 `$C1:CFAF`**(캡처 밖, cluster 시작 CFDC 앞)였다. 근본원인=포인터 카탈로거가 조건분기 한 갈래만 걸어 `LDA #$CFAF`(@$C1:7E24) 누락. 수정=dialogue.json에 엔트리(id 673)+pointer_map c1_ui에 `[0x17E25,0xCFAF,"lda"]` 추가. 674/674 무손실·어드벤처 위치보존 0회귀·실기 한글 확인.
   - 🔧 **트레이스 도구**: `scripts/lua/trace_field_src.lua` — 파서/씬표/디코더 3경로 동시 후킹, 화면 P/S/D 카운터로 어느 엔진 발화인지 육안 판별. **맵 NPC 갭 추적에 재사용**.
   - 🔴 **잔여 = 광역 "월드맵/맵 NPC 대사"**: $C1 메뉴 서브시스템($7F8044 범용 표시루틴, 9세터 전수대조=CFAF만 누락으로 **닫힘**)이 아닌 **별도 표시경로**다. 방법=맵 NPC 대화 화면에서 `trace_field_src.lua` 무장 후 대사 띄우고 P/S/D+소스주소 캡처 → 오프라인 디코드 → 갭 열거([[npc-field-dialogue-gap]]).
5. **(검수 보류)** 축약發 어색한 말투·토우키치 지요 우세 일부 — **최종 전수 검수**에서 손봄(사용자 세밀 검수 예정).
6. **그래픽 에셋 주입은 보류** — 사용자 수작업 검수·캡처 중. **모든 대사 번역 완료 후 사용자 요청 시**에만.

> 순서: 4(월드맵/맵 NPC 대사 발굴·번역, System ①) → 최종 검수. 반복 disasm/축약은 Codex 분산([[dual-ai-translation-workflow]]·docs/16 §6).
