...
## 🔴 다음 최우선 (2026-07-20 사용자 스테이지8 피드백)
1. **토우키치 말투 다양화** — 모든 상황 `~옵쇼` 고정이 어색. **맥락별 간신/하인/마당쇠/돌쇠/도련님 톤**으로 다양화:
   `~사옵니다/~하지요/~옵쇼/~굽쇼/~련지요/~나이다/~하옵고/~라네/~다네/~아뢰옵니다/~하옵니다/~굽쇼쇼쇼쇼/~지요/~요오?` 등.
   → glossary §1.5 토우키치 항목 개정 + **토우키치 대사 전수 보수**(Codex 배치, 완전 하네스).
2. **불필요 공백 들여쓰기 정리** — 축약 과정서 생긴 쓸데없는 전각/반각 공백이 전체 대사에 산발적. 전수 검출·정리.
3. **선택 분기 전 대화·선택지 대사 미번역** — cmd0x20 중첩 **~28런**(선택지/프롬프트). `out/cmd20_untranslated.json`. 완전 하네스로 번역.
4. **포메이션 설정 시 레츠 대사 미번역** — 우리 씬표 밖(**월드맵 대사** 추정). System ① 월드맵 대사 발굴 때 함께 찾기([[npc-field-dialogue-gap]]).
5. **(검수 보류)** 축약發 어색한 말투 일부 — **최종 전수 검수**에서 손봄(사용자 세밀 검수 예정).
6. **그래픽 에셋 주입은 보류** — 사용자가 타일/스프라이트 수작업 검수·캡처 중. **모든 대사 번역 완료 후 사용자 요청 시**에만.

> 순서: 위 1~4 보수 → 스토리 파이프라인 외 대사(맵 NPC 등, System ①) 번역.
