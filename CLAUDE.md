...
| 추출·완전성·포인터 | ✅ 완료 | 정적 대사 681, 어드벤처 메시지 1,715, 월드맵 350문자열, 필드/NPC 텍스트런 1,411(고유 1,340) 카탈로그와 역검증 경로 확정. 상세는 [docs/07](docs/07-dialogue-completeness.md)·[docs/08](docs/08-adventure-text-engine.md)·[docs/19](docs/19-worldmap-quiz-text.md)·[docs/20](docs/20-field-npc-hidden-records.md) |
| 빌드·검증 | 🟡 통합 빌드 완료·실기 QA 진행 | `build_all.py`가 정적·그래픽·어드벤처·필드/NPC·월드맵·SJIS를 **원본과 동일한 2MB ROM 내부**에 통합하고 체크섬 및 BPS를 생성. 정적 681, 어드벤처 228씬, 필드 685레코드, 월드맵 350문자열 역검증 PASS. 어드벤처 전편 실기 완주 완료, 필드 대표 장소 실기 QA 대기 |
