...
문서별 현재 정본·역사 기록·자동 생성 감사의 구분은 [docs 문서 지도](docs/README.md)를 따른다.
...
| round-trip 검증 | ✅ 완료 | 정적 681/681과 각 압축·폰트·포인터 컨테이너의 재해제·역디코드·최종 ROM 회귀를 통합 빌드에서 검사 |
...
| 폰트 경로 | ✅ 확정 | 본문은 ROM 뱅크 `$CA`의 16×16 1bpp VWF, 시작·SJIS·소형 메뉴는 `$D9` 계열 문맥별 글꼴을 사용한다. 각 소비 경로와 리다이렉트는 [docs/03](docs/03-font-survey.md)·[docs/11](docs/11-sjis-menu.md)·[docs/12](docs/12-sjis-ui-hangul.md)·[docs/18](docs/18-menu-tile-font-labels.md)에 확정했다. |
| 훅·재배치 | ✅ 완료 | 시스템별 포인터 재배치와 필요한 ASM 훅을 빌드 스크립트에 통합했다. 소형 직접타일 메뉴는 초기 마커 훅 대신 문맥별 글꼴 재압축·포인터 리다이렉트를 사용한다. |
| 수용량·ROM 크기 | ✅ 완료 | 글리프·압축 슬롯·재배치 용량을 빌드에서 검사하며, 최종 ROM은 원본과 동일한 헤더리스 2MiB를 유지한다. |
| PoC (한글 화면 표시) | ✅ 통과 | **실기 화면에 한글 렌더 확인**("부천 레트로 흥해라"). 실제 경로(스크립트 $C7→파서 $C1:9554→글리프 페치→폰트 시트 $CA) 그대로. 폰트=`assets/fonts/body/x12y12pxMaruMinyaHangul.ttf`(OFL), 도구=`cargo run -- poc-font`(TTF 래스터+시트 주입+per-글리프 폭/공백). base03 인코더 검증됨 → [docs/05-poc-hangul-font.md](docs/05-poc-hangul-font.md) |
...
| 빌드·검증 | ✅ v0.9 배포·실기 QA 완료 | `build_all.py`가 정적·그래픽·어드벤처·필드/NPC·월드맵·SJIS·소형 메뉴·Result·포메이션·능력치·개러지·경기 HUD·**VICTORYS 에피소드 인터미션 로고**·**챕터 인트로 2bpp 제목 10개**·**실제 엔딩 크레딧/베스트타임 45행과 현지화 메시지 12행**을 **원본과 동일한 2MB ROM 내부**에 통합한다. 정적 681, 어드벤처 **235씬·1,782메시지**, 필드 685레코드, 월드맵 350문자열, Result 선수 범위표 110/110·표시 이름 61종·물리 타일 공유 0, 인트로 그래픽 10/10, 경기 HUD와 엔딩 전용 글꼴·스트림 역검증 PASS. 완역–실삽입 조정문은 959건이며 원문·완역·삽입문을 분리 보존한다. 메인스토리 전편·대표 필드/NPC·승인 그래픽 실기 QA를 완료했고, v0.9 xdelta 역적용 결과가 통합 ROM과 바이트 일치한다. 계획된 잔여 작업은 없으며 이후 제보는 유지보수로 처리한다. |

## 디렉토리 구조

```
assets/
  fonts/body/       16×16 본문 글꼴
  fonts/small/      8×8 메뉴·엔딩 글꼴
  graphics/         화면·서브시스템별 승인 그래픽과 작업 템플릿
  screenshots/      문서용 선별 스크린샷
  translations/     기계 판독 가능한 번역·주소 SSOT
  translation_guide/ 고유명사·용어 통일
docs/
  history/          폐기 설계·병합 전 구현의 역사 기록
  worklogs/         작업 브리핑·세션 인계·시행착오
  01-24*.md         현재 구현 명세·감사 문서
scripts/
  *.py              통합 빌드와 분석·추출·검증 진입점
  lua/capture/      Mesen2 덤프·스크린샷 수집
  lua/probes/       단발성 상태 조사
  lua/traces/       실행·DMA·주소 추적
src/                Rust 파이프라인
roms/ out/ tmp/     원본·빌드 산출물·임시 파일 (gitignore)
```
