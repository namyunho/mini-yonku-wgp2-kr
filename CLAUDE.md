...
| 폰트 경로 | ✅ 확정 | **본문 폰트 = ROM 뱅크 $CA(`0x0A1137`), 16×16 1bpp(32B/글리프) 비압축, VWF.** 메뉴 소형 폰트 = `$D9:0000` LZSS 자원의 앞 4KB(8×8 2bpp). 월드맵·튜토리얼 문맥 포인터 교체 방식 실기 검증 완료 → [docs/03-font-survey.md](docs/03-font-survey.md), [docs/19](docs/19-menu4-context-font.md) |
...
| 추출·완전성·포인터 | ✅ 완전성 닫힘·c1 재추출 완료 | **파서 `$C1:9554` 호출처 7곳 전수 역추적 → 정적 텍스트 뱅크 = $C7/$D0/$C1 셋 권위 확정**(+$7E WRAM 조립버퍼, $00 클리어; 그 외 뱅크 0). **확정 정적 대사 673**: c7_race 232($C7:89E2)·**d0_story 404**($D0:C80B)·**c1_ui 37**(4클러스터 c1_setting $C868·11/c1_machines $C981·6/c1_garage $CE53·6/c1_formation $CFDC·14 — 이전 c1_form 15는 포메이션 클러스터만 본 오류, 정정). `dialogue.json` 재생성·**673 전량 라운드트립 통과**(raw==ROM & encode(parse)==raw). **전 블록 포인터 카탈로그** 확보(VM opcode·ROM 테이블·인라인 즉치): c7=VM 77+테이블 $C7:A1AD ×87, d0=테이블 26개(터치콜 $D0:C778, 스토리 $D0:F78B군)+인라인 71, c1=테이블 $C1:C501/CF90+인라인 26. 산출 `assets/translations/pointer_catalog.json`, 생성기 `scripts/final_catalog.py` → [docs/07-dialogue-completeness.md](docs/07-dialogue-completeness.md). 글리프표 1008 유지 → [docs/06](docs/06-dialogue-extraction.md). 다음: 번역, 재삽입(3경로 포인터 패치) |
| 빌드·검증 | ⬜ 미착수 | BPS 배포, Mesen2/bsnes 검증 |
...
- 배포는 원본에서 패치 ROM 생성 후 **BPS 차분**(flips)으로. 원본·패치 ROM은 비커밋.
