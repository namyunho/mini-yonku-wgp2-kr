...
| 추출·완전성·포인터 | ✅ 완료 | 정적 대사 681, 어드벤처 메시지 1,724, 월드맵 350문자열, 필드/NPC 텍스트런 1,411(고유 1,340) 카탈로그와 역검증 경로 확정. 상세는 [docs/07](docs/07-dialogue-completeness.md)·[docs/08](docs/08-adventure-text-engine.md)·[docs/19](docs/19-worldmap-quiz-text.md)·[docs/20](docs/20-field-npc-hidden-records.md) |
| 빌드·검증 | 🟡 통합 빌드 완료·실기 QA 진행 | `build_all.py`가 정적·그래픽·어드벤처·필드/NPC·월드맵·SJIS·소형 메뉴·Result·포메이션·능력치·개러지·경기 HUD·**VICTORYS 엔딩 로고**와 **챕터 인트로 2bpp 제목 10개**를 **원본과 동일한 2MB ROM 내부**에 통합하고 체크섬 및 BPS를 생성. 정적 681, 어드벤처 231씬·1,724메시지, 필드 685레코드, 월드맵 350문자열, 인트로 그래픽 10/10, 경기 HUD 라벨 7타일 + 일본어 윗행 잔재 4타일, 엔딩 로고 CHR·타일맵·로더 2곳 역검증 PASS. 어드벤처 전편 실기 완주 완료, 신규 승인 그래픽 실기 QA 진행 중 |
